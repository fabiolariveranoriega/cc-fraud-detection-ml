...
from .config import device, NUM_TRIALS, NUM_FOLDS, SEED
from .data import CreditCardData
from .graph import CreditCardGNN
...
def main(path:str):
    data = CreditCardData(path)
    v0_mapping, v1_mapping = data.get_mappings()
    graph_data = CreditCardGNN()
    G = graph_data.create_graph(data, v0_mapping, v1_mapping)
    data = graph_data.to_torch(G)
